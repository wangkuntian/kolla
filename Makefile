UV ?= uv run
KOLLA_BUILD ?= $(UV) kolla-build

BASE_DISTRO ?= openeuler
OPENSTACK_RELEASE ?= 2023.1
BASE_IMAGE ?= openeuler-24.03-lts
BASE_TAG ?= latest
IMAGE_TAG ?= $(OPENSTACK_RELEASE)
...
.PHONY: help build-all build-image build-% start-registry push-image push-all registry-list
